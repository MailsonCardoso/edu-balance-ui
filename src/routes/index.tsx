  ...
  return (
    <>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-4 mb-6">
        <StatCard label="Recebido no mês" value={brl(dashboardStats.totalRecebido)} icon={<Wallet className="size-5" />} tone="success" trend="+12% vs mês anterior" />
        <StatCard label="Em aberto" value={brl(dashboardStats.totalAberto)} icon={<CalendarClock className="size-5" />} tone="warning" trend="A receber neste mês" />
        <StatCard label="Vencido" value={brl(dashboardStats.totalVencido)} icon={<AlertCircle className="size-5" />} tone="destructive" trend="23 parcelas em atraso" />
        <StatCard label="Alunos ativos" value={dashboardStats.alunosAtivos} icon={<Users className="size-5" />} tone="info" trend="Matriculados" />
        <StatCard label="Inadimplentes" value={dashboardStats.alunosInadimplentes} icon={<UserX className="size-5" />} tone="destructive" trend="12% da base" />
      </div>

      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 mb-4">
        <ChartCard title="Receita mensal" subtitle="Comparativo com a meta">
          <ResponsiveContainer>
        ...
        <ChartCard title="Receita anual" subtitle="Evolução nos últimos anos">
          <ResponsiveContainer>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        <ChartCard title="Inadimplência por período" subtitle="Percentual mensal">
          <ResponsiveContainer>
        ...
        <ChartCard title="Evolução de pagamentos" subtitle="Pagos vs pendentes no mês">
          <ResponsiveContainer>
            <AreaChart data={evolucaoPagamentos}>
              ...
              <Area type="monotone" dataKey="pagos" stackId="1" stroke="var(--success)" fill="var(--success)" fillOpacity={0.3} />
              <Area type="monotone" dataKey="pendentes" stackId="1" stroke="var(--warning)" fill="var(--warning)" fillOpacity={0.3} />
            </AreaChart>
          </ResponsiveContainer>
        </ChartCard>
      </div>
